...
function read_extensible_array_chunks(f::JLDFile, v::Array, dataspace, rr,
    ...
    version == 0 || throw(UnsupportedVersionException("Unsupported Extensible Array version $version"))

    client_id = jlread(f.io, UInt8)
    element_size = jlread(f.io, UInt8)
    max_nelmts_bits = jlread(f.io, UInt8)
    index_blk_elmts = jlread(f.io, UInt8)
    data_blk_min_elmts = jlread(f.io, UInt8)
    secondary_blk_min_data_ptrs = jlread(f.io, UInt8)
    max_dblk_page_nelmts_bits = jlread(f.io, UInt8)

    # Read Length-size fields (8 bytes each per superblock)
    num_secondary_blks = jlread(f.io, UInt64)
    secondary_blk_size = jlread(f.io, UInt64)
    num_data_blks = jlread(f.io, UInt64)
    data_blk_size = jlread(f.io, UInt64)
    max_index_set = jlread(f.io, UInt64)
    nelmts = jlread(f.io, UInt64)
    # Index Block Address (Offset-size field - 8 bytes)
    index_blk_addr = jlread(f.io, RelOffset)

    # Skip checksum
    skip(f.io, 4)

    # Read index block
    if !isnothing(index_blk_addr) && index_blk_addr.offset != typemax(UInt64)
